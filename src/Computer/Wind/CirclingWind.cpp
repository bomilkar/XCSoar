...
CirclingWind::NewSample(const MoreData &info, const CirclingInfo &circling)
{
  ...
  if (!active) {
    ...
    if (useable_tas && !real_tas) LogString("CirclingWind: TAS not usable!");
  ...
  // use turn rate info from gyroscope is better at higher winds
  if (info.gyroscope.available && info.gyroscope.real) {
    trSource = 'g';
    sample.turn_rate = Angle::Degrees(sqrt(Square(info.gyroscope.turnRateY)+Square(info.gyroscope.turnRateZ)));
    // if ((info.gyroscope.turnRateY+info.gyroscope.turnRateZ) < 0) sample.turn_rate *= -1.0;
    if (info.gyroscope.turnRateZ < 0) sample.turn_rate *= -1.0;
  } else {
    trSource = 't';
    sample.turn_rate = (info.track - last_track).AsDelta();
  ...
  if (suspend-- <= 0) { // avoid WindCalc for every single sample.
    ...
    size_t idx;
    current_circle = Angle::Zero();
    Angle turnrate_sum = Angle::Zero();
    for (idx=1; (idx < samples.size()) && !fullCircle; idx++) {
      current_circle += (samples[idx-1].track - samples[idx].track).AsDelta();
      turnrate_sum += samples[idx-1].turn_rate;
      fullCircle = current_circle.Absolute() > Angle::FullCircle();
      // printf ("X,%1.2f,%1.2f\n",current_circle.Degrees(),turnrate_sum.Degrees());
    }
    const auto n_samples = idx; // in the full circle

    // how big is the change in turn rate?
    if (fullCircle && (n_samples > 8)) {
      const Angle turnrate_avg = turnrate_sum / (n_samples-1);
      if (false) {
        char line_buf[200];
        snprintf (line_buf,sizeof(line_buf),"TRsum,%c,%1.2f,%1.2f,%d,%1.2f",
          trSource,turnrate_sum.Degrees(),current_circle.Degrees(),(int)n_samples,turnrate_avg.Degrees());
        LogString(line_buf);
      }

      double max_turnrate_deviation = 0;
      for (size_t i=1; i < n_samples;i++) {
        const Angle v = samples[i].turn_rate.AsDelta() - turnrate_avg;
      ...
      if (false) {
        char line_buf[200];
        snprintf (line_buf,sizeof(line_buf),"TR_qm,%1.2f",circle_quality_metric);
        LogString(line_buf);
      }
      // printf ("avg,%1.2f,%1.2f,%ld\n",turnrate_avg.Degrees(),x_turnrate_avg,n_samples);
...
CirclingWind::Result
CirclingWind::CalcWind(double quality_metric, const size_t n_samples, const Angle circle)
{
  char line_buf[200];
  ...
  for (size_t i = 1; i < n_samples; i++)
    if (abs(samples[i-1].time - samples[i].time - avg_step_width) > (avg_step_width * 0.05))
      {
      // LogString("CirclingWind: step width not sufficiently uniform");
      return Result(0);
      }
  ...
  /**************************
  if (!((last_step_fraction_excess >= 0) && (last_step_fraction_excess <= 1))) {
    snprintf (line_buf,sizeof(line_buf),"assert Fract,%1.3f,%1.3f,%1.3f,%1.3f",
      circle.Degrees(),last_track_change.Degrees(),excess_circle.Degrees(),last_step_fraction_excess);
    LogString(line_buf);
  }
  ***************************/
...
unsigned int
CirclingWind::EstimateQuality(const double circle_quality, const double fitCosine_quality,  const double wind_speed)
...
double
CirclingWind::FitCosine(const size_t n_samples, const double amplitude, const double offset, const Angle phase)
